...
write.csv(pbmc.markers, "~/Desktop/P0_genes_subgroups_edited.csv")
```
#P1 
```{r}
matrix_dir = "~/Downloads/p1/count/sample_feature_bc_matrix/"
...
#add meta separately 
x2 <- rep("P1", each= 897)
pbmc_1 <- AddMetaData(object= pbmc, metadata = x2, col.name= "time")
```
...
```{r}
pbmc_1 <- RunUMAP(pbmc_1, dims = 1:5)
...
pbmc.markers %>%
    group_by(cluster) %>%
    slice_max(n = 2, order_by = avg_log2FC)
write.csv(pbmc.markers, "~/Desktop/P1_genes_subgroups.csv")
```
#P6
```{r}
matrix_dir = "~/Downloads/p6/count/sample_feature_bc_matrix/"
barcode.path <- paste0(matrix_dir, "barcodes.tsv.gz")
features.path <- paste0(matrix_dir, "features.tsv.gz")
matrix.path <- paste0(matrix_dir, "matrix.mtx.gz")
mat <- Matrix::readMM(file = matrix.path)
...
#add meta separately 
x2 <- rep("P7", each= 1794)
pbmc_3 <- AddMetaData(object= pbmc, metadata = x2, col.name= "time")
```
...
```{r}
pbmc_3 <- FindNeighbors(pbmc_3, dims = 1:10)
pbmc_3 <- FindClusters(pbmc_3, resolution = 0.25)
```

```{r}
pbmc_3 <- RunUMAP(pbmc_3, dims = 1:3)
...
pbmc.markers %>%
    group_by(cluster) %>%
    slice_max(n = 2, order_by = avg_log2FC)
write.csv(pbmc.markers, "~/Desktop/P6_genes_subgroups.csv")
```

#P7
```{r}
matrix_dir = "~/Downloads/p7/count/sample_feature_bc_matrix/"
barcode.path <- paste0(matrix_dir, "barcodes.tsv.gz")
features.path <- paste0(matrix_dir, "features.tsv.gz")
matrix.path <- paste0(matrix_dir, "matrix.mtx.gz")
mat <- Matrix::readMM(file = matrix.path)
...
#add meta separately 
x2 <- rep("P7", each= 1617)
pbmc_4 <- AddMetaData(object= pbmc, metadata = x2, col.name= "time")
```
...
#also edited to higher dims (from 3 to 13) and chnage in resolution (from 0.5 to 0.25)
```{r}
pbmc_4 <- FindNeighbors(pbmc_4, dims = 1:10)
pbmc_4 <- FindClusters(pbmc_4, resolution = 0.25)
```

```{r}
pbmc_4 <- RunUMAP(pbmc_4, dims = 1:3)
