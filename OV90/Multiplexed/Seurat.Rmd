---
title: "Multiplexed_05302023_Ov90"
output: html_document
date: "2023-07-11"
---

For this experiment, we are looking at the data from the multiplexed time points 
from Ov90. The time points included are P0, P1, P6, P7.  They are sequenced where each should have 5k cells. Actual Totals according to cell ranger were: 
#P0
```{r}
library(dplyr)
library(Seurat)
...
```{r}
matrix_dir = "~/Single_Cell/Ov90/Multiplexed/Cell_ranger_output/P0/"
...
write.csv(pbmc.markers, "~/Single_Cell/Ov90/Multiplexed/PBMC_Markers/P0_genes_subgroups.csv")
```
#P1 
```{r}
matrix_dir = "~/Single_Cell/Ov90/Multiplexed/Cell_ranger_output/P1/"
barcode.path <- paste0(matrix_dir, "barcodes.tsv.gz")
features.path <- paste0(matrix_dir, "features.tsv.gz")
matrix.path <- paste0(matrix_dir, "matrix.mtx.gz")
mat <- Matrix::readMM(file = matrix.path)
...
pbmc_1 <- FindClusters(pbmc_1, resolution = 0.87)
```

```{r}
pbmc_1 <- RunUMAP(pbmc_1, dims = 1:35)
...
pbmc.markers %>%
    group_by(cluster) %>%
    slice_max(n = 2, order_by = avg_log2FC)
write.csv(pbmc.markers, "~/Single_Cell/Ov90/Multiplexed/PBMC_Markers/P1_genes_subgroups.csv")
```
#P6
```{r}
matrix_dir = "~/Single_Cell/Ov90/Multiplexed/Cell_ranger_output/P6/"
barcode.path <- paste0(matrix_dir, "barcodes.tsv.gz")
features.path <- paste0(matrix_dir, "features.tsv.gz")
matrix.path <- paste0(matrix_dir, "matrix.mtx.gz")
mat <- Matrix::readMM(file = matrix.path)
...
```{r}
pbmc_3 <- FindNeighbors(pbmc_3, dims = 1:40)
pbmc_3 <- FindClusters(pbmc_3, resolution = 0.87)
```

```{r}
pbmc_3 <- RunUMAP(pbmc_3, dims = 1:35)
...
pbmc.markers %>%
    group_by(cluster) %>%
    slice_max(n = 2, order_by = avg_log2FC)
write.csv(pbmc.markers, "~/Single_Cell/Ov90/Multiplexed/PBMC_Markers/P6_genes_subgroups_no_subset.csv")
```

#P7
```{r}
matrix_dir <- "~/Single_Cell/Ov90/Multiplexed/Cell_ranger_output/P7/"
barcode.path <- paste0(matrix_dir, "barcodes.tsv.gz")
features.path <- paste0(matrix_dir, "features.tsv.gz")
matrix.path <- paste0(matrix_dir, "matrix.mtx.gz")
mat <- Matrix::readMM(file = matrix.path)
...
```{r}
pbmc_4 <- FindNeighbors(pbmc_4, dims = 1:40)
pbmc_4 <- FindClusters(pbmc_4, resolution = 0.87)
```

```{r}
pbmc_4 <- RunUMAP(pbmc_4, dims = 1:35)
...
pbmc.markers %>%
    group_by(cluster) %>%
    slice_max(n = 2, order_by = avg_log2FC)
write.csv(pbmc.markers, "~/Single_Cell/Ov90/Multiplexed/PBMC_Markers/P7_genes_subgroups_no_subset.csv")
```

#Merging 
```{r}
pbmc.combined <- merge(pbmc, y = c(pbmc_1, pbmc_3, pbmc_4), add.cell.ids = c("P0","P1","P6","P7"), project = "OV90_multiplexed")
pbmc.combined
```
...
pbmc.markers %>%
    group_by(cluster) %>%
    slice_max(n = 2, order_by = avg_log2FC)
write.csv(pbmc.markers, "~/Single_Cell/Ov90/Multiplexed/PBMC_Markers/Ov90_combined_genes_subgroups.csv")
```

```{r}
pbmc.combined <- merge(pbmc, y = c(pbmc_4), add.cell.ids = c("P0", "P7"), project = "Ov90_multiplexed")
pbmc.combined
```
...
pbmc.markers %>%
    group_by(cluster) %>%
    slice_max(n = 2, order_by = avg_log2FC)
write.csv(pbmc.markers, "~/Single_Cell/Ov90/Multiplexed/PBMC_Markers/Ov90_P0_P7_genes_subgroups.csv")
```

```{r}
pbmc.combined <- merge(pbmc, y = c(pbmc_3), add.cell.ids = c("P0", "P6"), project = "Ov90_multiplexed")
pbmc.combined
```
...
pbmc.markers %>%
    group_by(cluster) %>%
    slice_max(n = 2, order_by = avg_log2FC)
write.csv(pbmc.markers, "~/Single_Cell/Ov90/Multiplexed/PBMC_Markers/OV90_P0_P6_genes_subgroups_no_subset.csv")
```

```{r}
pbmc.combined <- merge(pbmc_1, y = c(pbmc_4), add.cell.ids = c("P1", "P7"), project = "Ov90_multiplexed")
pbmc.combined
```
...
pbmc.markers %>%
    group_by(cluster) %>%
    slice_max(n = 2, order_by = avg_log2FC)
write.csv(pbmc.markers, "~/Single_Cell/Ov90/Multiplexed/PBMC_Markers/Ov90_P1_P7_genes_subgroups_no_subset.csv")
```


```{r}
pbmc.combined <- merge(pbmc, y = c(pbmc_1), add.cell.ids = c("P0", "P1"), project = "Ov90_multiplexed")
pbmc.combined
```
...
pbmc.markers %>%
    group_by(cluster) %>%
    slice_max(n = 2, order_by = avg_log2FC)
write.csv(pbmc.markers, "~/Single_Cell/Ov90/Multiplexed/PBMC_Markers/OV90_P0_P1_genes_subgroups_no_subset.csv")
```